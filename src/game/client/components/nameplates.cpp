...
void CNamePlates::RenderNameplate(vec2 Position, const CNetObj_PlayerInfo *pPlayerInfo, float Alpha, bool ForceAlpha)
{
	...
	if((!pPlayerInfo->m_Local || g_Config.m_ClNameplatesOwn) && g_Config.m_ClNameplates)
	{
		...
		{
            YOffset -= FontSizeClan;
			if(NamePlate.m_SkinTextContainerIndex.Valid())
				TextRender()->RenderTextContainer(NamePlate.m_SkinTextContainerIndex, TColor, TOutlineColor, Position.x - TextRender()->GetBoundingBoxTextContainer(NamePlate.m_SkinTextContainerIndex).m_W / 2.0f, YOffset);
		}
	}
